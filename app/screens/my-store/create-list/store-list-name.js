...
const NewMyStoreListName = ({navigation, route}) => {
  ...
  const onSubmit = () => {
    if (listName) {
      ...
      DataService.createShoppingList(listName, client.slug).subscribe(
        async data => {
          setIsLoading(false);
          ...
          if (status) {
            ...
            navigation.navigate('MyStoreAddItems', {
              client,
              shoppingListSlug,
              isNewList: true,
            });
...
export default NewMyStoreListName;
