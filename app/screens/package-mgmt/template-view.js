...
export const TemplateView = forwardRef(({ formikProps, template, packageTemplate }, ref) => {
    const [isShowTemplates, setIsShowTemplates] = useState(false);
    let filterData = template.filter(item => !formikProps.values.deliverables.includes(item));
    ...
    const updateTemplateList = (formikProps, index) => {
        setTemplateList(prev => {
            ...
            formikProps.values.deliverables.push(removedItem);
            return temp;
        });
    };
            ...
            <>
                ...
                {isShowTemplates && templateList.map((item, index) => (
                    <TouchableOpacity onPress={() => {
                        formikProps.setFieldValue('deliverables', [item, ...formikProps.values.deliverables]);
